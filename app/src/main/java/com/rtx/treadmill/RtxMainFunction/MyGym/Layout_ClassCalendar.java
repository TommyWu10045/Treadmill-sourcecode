...
public class Layout_ClassCalendar extends Rtx_BaseLayout {
    ...
    protected void vSetDate(int iWeek)
    {
        ...
        mMainActivity.mMainProcTreadmill.myGymProc.vSetSearchDate(cal_Start,cal_End);

        if(iModeSelect == MODE_CLASS)
        {
            mMainActivity.mMainProcTreadmill.myGymProc.vSetNextState(MyGymState.PROC_CLOUD_GET_CLASS_INFO_LIST);
            vEnableArrowButton(true);
        }
        else
        //if(iModeSelect ==MODE_MY_CLASS)
        {
            mMainActivity.mMainProcTreadmill.myGymProc.vSetNextState(MyGymState.PROC_CLOUD_GET_MY_CLASS_INFO_LIST);
    ...
    private boolean check_Login()
    {
        ...
        else
        {
            ...
            mMainActivity.dialogLayout_Login.fillerTextView_Login.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    mMainActivity.mMainProcTreadmill.myGymProc.vMainChangePage(MainState.PROC_LOGIN);
