...
public class Main01Layout extends Rtx_BaseLayout {
    ...
    public void init_View() {
        ...
        if (t_date_data == null) {
            t_date_data = new RtxTextView(mContext);
        }
        if (i_heart_rate == null) {
            i_heart_rate = new RtxImageView(mContext);
        }
        if (t_heart_rate == null) {
            t_heart_rate = new RtxTextView(mContext);
        }
        if (i_edit_pen == null) {
            i_edit_pen = new RtxImageView(mContext);
        }
        if (i_edit_cancel == null) {
            i_edit_cancel = new RtxImageView(mContext);
        }
        if (i_info == null) {
            i_info = new RtxImageView(mContext);
        }
        if (i_next == null) {
            i_next = new RtxImageView(mContext);
        }
        if (i_bdinfo == null) {
            i_bdinfo = new RtxImageView(mContext);
        }
        if (i_muscle == null) {
            i_muscle = new RtxImageView(mContext);
        }
        if (i_bodyfat == null) {
            i_bodyfat = new RtxImageView(mContext);
        }
        if (t_muscle == null) {
            t_muscle = new RtxTextView(mContext);
        }
        if (t_bodyfat == null) {
            t_bodyfat = new RtxTextView(mContext);
        }
        if (t_data == null) {
...
//      body info
    ...
    class ButtonListener implements OnClickListener {
        ...
        public void onClick(View v) {
            if (v == i_edit_pen) {
                MyLog.d("===>  Main01Layout i_edit_pen click");
                vEditPen_icon();
            } else if (v == i_edit_cancel) {
                vEditPen_cancel_icon();
            } else if (v == i_info) {
                vShowInfoDialog();
            } else if (v == imageView_BackHome) {
                vClickMain();
            } else if (v == i_next) {
                vClicknext();
            } else if (v == i_bdinfo) {
