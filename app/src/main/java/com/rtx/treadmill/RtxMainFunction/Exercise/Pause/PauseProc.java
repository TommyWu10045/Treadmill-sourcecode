...
import com.rtx.treadmill.RtxMainFunction.Exercise.ChoiceItem.ChoiceItemState;
...
public class PauseProc {
    ...
    private void vProc_Init() {
...

        {
            ...
            mMainActivity.mMainProcTreadmill.choiceItemProc.vSetState(ChoiceItemState.PROC_IDLE);
        }
    ...
    public void vProc_Reset_Mode01_flash()
    {
        ...
        if(icount >= itotal)
        {
            vSetState(PauseState.PROC_PAUSE_RESET1_RESUME);
        }
        else {
            mMainActivity.mUI_Handler.post(new Runnable() {
                @Override
