...
//				if (HeartRateControl.ikey_incline_count == HeartRateControl.ikey_delay_count)
	...
	public void v_init_Cooldown_data1(float ftime)
	{
		...
		float fspeed ;
		float fincline ;
		float[] fspeed_array = new float[imax*2] ;
		...
		fspeed = Exercisefunc.fGetCurrentSpeed();
		fincline = EngSetting.f_Get_Min_Incline();

		for(iLoop = 0; iLoop < (imax * 2); iLoop++) {
			if((iLoop % 2) == 0)
			{
				fspeed = fspeed * CoolDownData.lcooldown_factor;
			}
			if(fspeed < CoolDownData.f_cooldown_min_speed)
			{
				fspeed = CoolDownData.f_cooldown_min_speed;
			}
			fspeed_array[iLoop] = fspeed;
		}
		...
		for(iLoop = 0; iLoop < imax; iLoop++) {
			...
			suart_cmd_orig.fspeed = fspeed_array[iLoop];
			suart_cmd_orig.fincline = fincline;
		...
		if (CoolDownData.list_real_setting.size() > 0) {
			...
			if (Uart_command != null) {
				...
				v_Speed_Change_infoshow(Uart_command.fspeed, CoolDownData.lbefore_ajust_time/1000);
			}
		}
	...
	private void v_Speed_Change_infoshow(float fspeed, float ftime)
	{
		String sdata;

		sdata = LanguageData.s_get_string(mContext, R.string.speed_decreases_to) + " "
				+ EngSetting.Distance.getValString(fspeed) + " "
	...
	private void v_CoolDown_Disable_infoshow()
	{
		...
		sdata = LanguageData.s_get_string(mContext, R.string.auto_speed_cool_down_disabled);
	...
	private void v_Speed_Change_Command(int index)
	{
	...
	private boolean Target_check_allmode(long ldiff)
    {
		...
		switch (CoolDownData.iCD_Get_status())
		{
			case 0x01 :
				...
				{
					v_Speed_Change_Command(CoolDownData.ilist_count);
	...
	public boolean vCoolDownfunc_setSpeed()
	{
		if(CoolDownData.list_real_setting.size() > CoolDownData.ilist_count) {
			//設定啟動速度
			v_Speed_Change_Command(CoolDownData.ilist_count);
