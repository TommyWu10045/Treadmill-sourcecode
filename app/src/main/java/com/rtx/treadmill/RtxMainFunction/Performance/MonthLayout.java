...
public class MonthLayout extends PFBaseLayout {
    ...
    private int istr_list[] = { R.string.distance, R.string.duration, R.string.calories, R.string.pace, R.string.heart_rate, R.string.watt };
    ...
    protected void init_CustomerView() {
        ...
        if(draw_image == null) { draw_image = new RtxImageView(mContext); }
        if(mfiller == null){
            mfiller = new RtxFillerTextView[imax];
            for(iLoop = 0; iLoop < imax; iLoop++){ mfiller[iLoop] = new RtxFillerTextView(mContext); }
        }
    }
    ...
    protected void init_CustomerEvent(){
        int iLoop;
        for(iLoop = 0; iLoop < imax; iLoop++){ mfiller[iLoop].setOnClickListener(mButtonListener); }
    }
    ...
    protected void add_CustomerView(){
        ...
        iw = 930;
        ih = 330;
        if(draw_view == null){ draw_view = new RtxDrawView(mContext, ix, iy, iw, ih); }
    ...
    private void v_item_select(int isel){
        ...
        i_item_choice = isel;
        if(scurr_data != null){ Refresh_All_data(); }
    }
    ...
    @Override
    protected void v_prev(){ vchoice_cal(1); }

    @Override
    protected void v_next(){ vchoice_cal(-1); }

    @Override
    protected void v_share(){ mMainActivity.showInfoDialog(mMainActivity.DIALOG_TYPE_SHARE,mMainActivity,mMainActivity.frameLayout_Base); }

    @Override
    protected void v_icon_click(View v){
        int iLoop;
        for(iLoop = 0; iLoop < imax; iLoop++){
            if(v == mfiller[iLoop]){ v_item_select(iLoop); break; }
        }
    }
    ...
    private void v_trans_show_data(String[] sdata){
        ...
        if(sdata == null){
            ...
            if(imonth < Common.LONG_MONTH_OF_YEAR.length){ smonth = LanguageData.s_get_string(mContext, Common.LONG_MONTH_OF_YEAR[imonth]); }
            stemp = smonth + " " + syear;
            t_date.setText(stemp);
        }
        v_draw_show(sdata);
    ...
    private void v_Refresh_draw(String stitle, int iday_max, int iitem){
        ...
        if(daystree.size() > 0){
            ...
            for(iLoop=0; iLoop < iday_max; iLoop++){
                ...
                //Log.e("EndersChen","SKEY["+skey+"]");
                value = daystree.get(skey);
                if(iLoop == 0){ sx[iLoop] = "1"; }
                else if(iLoop == (iday_max - 1)){ sx[iLoop] = Rtx_TranslateValue.sInt2String(iday_max); }
                else{ sx[iLoop] = "."; }

                if(value != null){
                    if(iitem == 1 || iitem == 3){
                        cCal=Rtx_Calendar.cStr2Calendar(value[iitem_list[iitem]], "HH:mm:ss");
                        //Log.e("EndersChen","VALUE["+value[iitem_list[iitem]].toString()+"]");
                        if(cCal != null){
                            ih=cCal.get(Calendar.HOUR);
                            im=cCal.get(Calendar.MINUTE);
                            is=cCal.get(Calendar.SECOND);
                        }
                        //Log.e("EndersChen","H["+ih+"]M["+im+"]S["+is+"]");
                        fy[iLoop]=ih*3600+im*60+is;
                    }else{
                        //Log.e("EndersChen","VALUE["+value[iitem_list[iitem]].toString()+"]");
                        fy[iLoop] = Rtx_TranslateValue.fString2Float(value[iitem_list[iitem]], 0);
                    }
                    if(fymax < fy[iLoop]){ fymax=fy[iLoop]; }
                }else{ fy[iLoop]=0; }
                if(iitem == 1 || iitem == 3){
                    //9為不會產生0:00的最小值
                    if(fymax < 9 * 60){ draw_view.vRtxDrawView_setAxisMaxMin(9 * 60, 0); }
                    else{ draw_view.vRtxDrawView_setAxisMaxMin(0, 0); }
                }else{
                    //5為分成五等分而不會產生四捨五入的顯示錯誤
                    if(fymax < 5){ draw_view.vRtxDrawView_setAxisMaxMin(5, 0); }
                    else{ draw_view.vRtxDrawView_setAxisMaxMin(0, 0); }
                }
            }
    ...
    private void v_draw_show(String[] sdata) {
        ...
        if (sdata == null) {
            ...
            if(i_item_choice < imax){
                if(i_item_choice == 1 || i_item_choice == 3){ draw_view.RtxDrawView_PF_Month("", "H:mm"); }
                else{ draw_view.RtxDrawView_PF_Month("", 0); }//PACE
    ...
    private void v_next_prev_check(){
        if(ichoice_max <= 1){
            ...
            if(ichoice_show == 0){ i_next.setEnabled(false); }
    ...
    private int iget_map_index(String skey){
        ...
        for(String key: keys){
            if(skey.compareTo(key) == 0){ break; }
            icount++;
        }
        if(icount != monthtree.size()){ iret = icount; }
        return iret;
    }
    ...
    private void vchoice_cal(int inum){
        ...
        if(ichoice_show < 0){
            ...
            if(monthtree.size() > 0){
                ...
                if(ichoice >= ichoice_max){ ichoice = ichoice_max - 1; }
