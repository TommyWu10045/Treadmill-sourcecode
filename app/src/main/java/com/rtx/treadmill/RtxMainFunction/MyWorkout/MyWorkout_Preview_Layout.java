...
public class MyWorkout_Preview_Layout extends Rtx_BaseLayout {
    ...
    private void init_View()
    {
...

        {
            ...
            if(preview_Chart == null)             { preview_Chart = new RtxExercisePreview(mContext,0); }
        }
    }
    ...
    private void add_Preview()
    {
...

        {
            ...
            preview_Chart.t_speed.setText(LanguageData.s_get_string(mContext,R.string.speed));
            preview_Chart.i_speed.setImageResource(R.drawable.speed_icon);

            preview_Chart.t_speed.setVisibility(VISIBLE);
            preview_Chart.i_speed.setVisibility(VISIBLE);
            preview_Chart.t_incline.setVisibility(VISIBLE);
            preview_Chart.i_incline.setVisibility(VISIBLE);
            preview_Chart.t_min.setVisibility(VISIBLE);
            preview_Chart.i_profile.vSet_iIncline_mode(0x02);
        }
    }
    ...
    private void vClickButton()
    {
        ...
        {
            mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetNextState(MyWorkoutState.PROC_CLOUD_ADD_INTERVAL);
        }
        else if(iMode == MODE_MODIFY)
        {
            mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetNextState(MyWorkoutState.PROC_CLOUD_UPDATE_INTERVAL);
        }
        else
        {
    ...
    {
        mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetNextState(MyWorkoutState.PROC_SHOW_PAGE_DETAILS);
    }

    private void vClickDeleteItem()
    {
        vShowDeleteDialog();
        //mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetNextState(MyWorkoutState.PROC_CLOUD_DELETE_INTERVAL);
    }

    private void vClickModifyItem()
    {
        mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetNextState(MyWorkoutState.PROC_SHOW_PAGE_DATALIST);
        mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetMode(MODE_MODIFY);
    }

    private void vClickEditNameButton()
    {
        mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetPreState(MyWorkoutState.PROC_SHOW_PAGE_PREVIEW);
    ...
    protected void vShowDeleteDialog()
    {
        ...
        mMainActivity.dialogLayout_Delete.fillerTextView_Delete.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetDeleteMode(0);
                mMainActivity.mMainProcTreadmill.myWorkoutProc.vSetNextState(MyWorkoutState.PROC_SHOW_DIALOG_DELETE);
            }
        });
    }
}
