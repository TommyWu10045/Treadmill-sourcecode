...
//      Item name
        ...
        for(iLoop = 0; iLoop < imax; iLoop++) {
            ...
            addRtxTextViewToLayout(t_show[iLoop], sdata, fsize, Common.Color.setting_word_blue, Common.Font.Relay_Black, Typeface.NORMAL, Gravity.LEFT|Gravity.CENTER_VERTICAL, ix, iy, iw, ih);
            if(iLoop == 0 || iLoop == 2)
            {
                iy += 100;
            }
...
//        unit and gender
    ...
    {
        mMainActivity.mMainProcTreadmill.settingProc.vSetNextState(SettingState.PROC_CLOUD_37_SET);
    }

    private void vClickName()
    {
        mMainActivity.mMainProcTreadmill.settingProc.vSetNextState(SettingState.PROC_SHOW_PROFILE_NAME);
    }

    private void vClickHeight()
    {
        mMainActivity.mMainProcTreadmill.settingProc.vSetNextState(SettingState.PROC_SHOW_PROFILE_HEIGHT);
    }

    private void vClickBirth()
    {
        mMainActivity.mMainProcTreadmill.settingProc.vSetNextState(SettingState.PROC_SHOW_PROFILE_BIRTH);
    }
