 ...
 */
public class BottomQuickStartLayout extends Rtx_BaseLayout {
    ...
    private int i_choice_list[][] = {//item choice name for normal
            {R.string.pace, R.string.avg_pace},
            {R.string.distance},
    ...
    public BottomQuickStartLayout(Context context, MainActivity mMainActivity){
        super(context);
    ...
    private void item_choice_show(int iLoop)
    {
        ...
        float fsize = fGet_Scale(20f);
        SimpleObj obj = ArrayObj[iLoop];

        if(obj != null && obj.i_item_type != 0) {
            ixshift = obj.ixcenter;
            Choice_UI_Info.vSet_Choice(ixshift-30, iy_bot, fsize, obj);
        }
    }
