...
abstract public class SummaryBaseLayout extends Rtx_BaseLayout {
    ...
    public int istr_list[] = {
            //item name
            R.string.cardio_type  ,  R.string.duration  ,  R.string.distance ,  R.string.calories ,
            R.string.avg_speed  ,  R.string.avg_watt  ,  R.string.avg_heart_rate ,  R.string.max_heart_rate
    };
    ...
    public void init_View()
    {
        ...
        if(t_smode == null) {
            t_smode = new RtxTextView(mContext);
        }
    }
...
//      detail
        ...
        for(iLoop = 0; iLoop < imax; iLoop++) {
            ...
            else if(iLoop == 4)
            {
                ...
                sdata = EngSetting.Distance.getValString(fval, 1);
                sunit = EngSetting.Distance.getSpeedUnitString(mContext).toLowerCase();
            }
            else if(iLoop == 5)
            {
                fval = ExerciseGenfunc.f_get_ave_watt();
...
//                fsize = 43.32f;
                ...
                addRtxTextViewToLayout(t_smode, sdata.toUpperCase(), fsize, Common.Color.physical_word_white, Common.Font.Relay_Black, Typeface.NORMAL
                        , Gravity.BOTTOM|Gravity.CENTER_HORIZONTAL, ix-20, iy-ih, iw+40, ih*2);
            }
            else
            {
                fsize = 28.32f;//By Alan
