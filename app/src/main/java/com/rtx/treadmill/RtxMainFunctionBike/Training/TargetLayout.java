...
public class TargetLayout extends Rtx_BaseLayout {
    ...
    public int istr_list[][] =  {
            {R.drawable.tr_time        ,       R.string.time    },
            {R.drawable.tr_distance    ,       R.string.distance     },
            {R.drawable.tr_calories    ,       R.string.calories   },
    };
...
//        menu
        sdata = LanguageData.s_get_string(mContext, R.string.set_a_goal);
...
//      weight icon
    ...
    private void vClickitem(int iLoop)
    {
        if(iLoop == 2)
        {
            mMainActivity.mMainProcBike.trainingProc.vSet_mode(0+mMainActivity.mMainProcBike.trainingProc.i_target_start);
            mMainActivity.mMainProcBike.trainingProc.vSetNextState(TrainingState.PROC_SHOW_CALORIES);
        }
        else if(iLoop == 0)
        {
            mMainActivity.mMainProcBike.trainingProc.vSet_mode(1+mMainActivity.mMainProcBike.trainingProc.i_target_start);
            mMainActivity.mMainProcBike.trainingProc.vSetNextState(TrainingState.PROC_SHOW_TIME);
        }
        else if(iLoop == 1)
        {
            mMainActivity.mMainProcBike.trainingProc.vSet_mode(2+mMainActivity.mMainProcBike.trainingProc.i_target_start);
