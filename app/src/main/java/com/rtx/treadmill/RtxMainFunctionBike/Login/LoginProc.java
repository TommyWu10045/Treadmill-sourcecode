...
import com.rtx.treadmill.BT.BT_login_Peripheral_Bike;
...
//                    if(!CloudDataStruct.CloudData_63.check_is_null())
    ...
    {
        mMainActivity.mMainProcBike.loginProc.vSetNextState(LoginState.PROC_EXIT);
...
//                    }
    ...
    public void v_open_bt(boolean enable)
    {
        ...
        {
            if(gbt.bt_class_bike == null)
            {
                gbt.bt_class_bike = new BT_login_Peripheral_Bike(mContext, this);
            }
            if(enable) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    gbt.bt_class_bike.onStart(mContext);
                }
            }
            else
            {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    gbt.bt_class_bike.onStop();
                }
            }
        }
        gbt.bt_state = enable;
    ...
    public void v_count_bt()
    {
        if(gbt.bt_state)
        {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                gbt.bt_class_bike.v_timeout_count();
            }
        }
    }
