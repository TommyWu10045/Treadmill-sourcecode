...
//                        mState=PerformanceState.PROC_SHOW_SESSION;
    ...
    {
        mMainActivity.mMainProcBike.performanceProc.vSetNextState(PerformanceState.PROC_EXIT);
...
//                        Log.e(TAG, "sessionlist[" + iLoop + "][" + iLoop1 + "]=" + slist[iLoop1]);
//                    }
//                }
//            }
//        }
...
//        }
    ...
    public void run() {
        if(tempState != mState)
        {
            //Log.e("Jerry", "[performanceProc] mState = " + mState);
            tempState = mState;
        }
...
//                smch_type = "Elliptical";
        ...
        bResult = CloudDataStruct.CloudData_11.Set_input(smch_type, s_dur_time, s_duration, s_mode, s_mode_name
    ...
    public Runnable CloudRunnable_Get_Session = new Runnable() {
        ...
        public void run()
        {
            ...
            {
                sdate = Rtx_Calendar.s_trans_DateTime_Str(2, "yyyy-MM-dd HH:mm:ss", null, null, 1, 0);
            }
            else {
                sdate = sessionlist.get(sessionlist.size() - 1)[Cloud_21_GET_EXC_REC.Output.CRE_DT];
            }
    ...
    public Runnable CloudRunnable_Del_Session = new Runnable() {
        ...
        public void run()
        {
            ...
            if(sessionlist.size() > iDel_Work_Seq) {
                ...
                if (sDel_Work_Seq != null) {
                    iServerResponse = mMainActivity.mCloudCmd.iCloudCmd_DelExercise_Session(sDel_Work_Seq);
                }
            }
    ...
    private void v_get_month_duration(int istart, int iend)
    {
        ...
        if(iend == icurr_year)
        {
            edate = Rtx_TranslateValue.sInt2String(iend, 4) + "-" + Rtx_TranslateValue.sInt2String(icurr_month, 2);
        }
        else
        {
            edate = Rtx_TranslateValue.sInt2String(iend, 4) + "-12";
        }
        ...
        if (iServerResponse == 11 || iServerResponse == 0) {
            ...
            if (iServerResponse == 11) {
                imonth_reload = 0;
            } else if (iServerResponse == 0) {
                imonth_reload = 1;
            }
        }
