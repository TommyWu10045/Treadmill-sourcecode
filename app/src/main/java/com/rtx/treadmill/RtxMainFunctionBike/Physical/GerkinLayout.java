...
public class GerkinLayout extends Rtx_BaseLayout {
    ...
    public void init_View()
    {
        ...
        if(i_heart == null)    { i_heart = new RtxImageView(mContext);     }
...
//        menu
        sdata = LanguageData.s_get_string(mContext, R.string.vo2);
...
//      t_step
    ...
    private void v_heart_stage_curr()
    {
        ...
        index = ExerciseData.ilist_count;

        if(index < ExerciseData.list_real_setting.size())
        {
            sdata = s_stage + Rtx_TranslateValue.sInt2String(index + 1);

            f_step[0].setText(sdata);

            ExerciseData.DEVICE_COMMAND Uart_Command = ExerciseData.list_real_setting.get(index);
        ...
        i_step[0][2].setVisibility(INVISIBLE);
        t_step[0][2].setVisibility(INVISIBLE);
    ...
    private void v_heart_stage_next()
    {
        ...
        index = ExerciseData.ilist_count + 1;
