...
public class RtxDoubleStringForTargetHistoryView extends RtxView {
    ...
    public RtxDoubleStringForTargetHistoryView(Context context) {
        super(context);
    ...
    private void drawCenter(Canvas canvas, Rect r, Paint paint, String str, int iGap, Paint paint_sub, String str_sub)
    {
        ...
        drawText(canvas, str, x, y, paint, true);
        ...
        drawText(canvas, str_sub, x_sub, y_sub, paint_sub, false);
    ...
    private void drawText(Canvas canvas, String str, int iX, int iY, Paint paint, boolean bParse)
    {
        if(!bParse)
        {
            canvas.drawText(str, iX, iY, paint);
        }
        else
        {
            int iLen = str.length();
            int iKeyIndex = str.indexOf(cKey);

            if(iKeyIndex != -1)
            {
                paint.setColor(iMain_clolor);
                canvas.drawText(str, 0, iKeyIndex+1, iX, iY, paint);
                Rect bounds_all = new Rect();
                paint.getTextBounds(str, 0, iLen, bounds_all);
                Rect bounds = new Rect();
                paint.getTextBounds(str, iKeyIndex + 1, iLen, bounds);
                paint.setColor(Common.Color.yellow_4);
                canvas.drawText(str, iKeyIndex + 1, iLen, iX + (bounds_all.width() - bounds.width()), iY, paint);
            }
            else
            {
                canvas.drawText(str, iX, iY, paint);
