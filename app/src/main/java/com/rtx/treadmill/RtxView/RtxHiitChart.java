 ...
 */
public class RtxHiitChart extends RtxImageView {
    ...
    public RtxHiitChart(Context context){
        super(context);
        mContext = context;
    }
    ...
    public void init(int iscale, int imin, int imax, float fscale, int ix, int iy, int iw, int ih){
        ...
        iViewWidth = iw;
        iViewHeight = ih;
        if(EngSetting.getUnit() == EngSetting.UNIT_METRIC){ fspeed_times = 1f; }
        else{ fspeed_times = EngSetting.km2mile; }
        init_paint();
        Create_Bitmap();
    }
    ...
    private boolean vSpeed_effect(int iLoop){
        ...
        if((ieffect & 0x01) == 0x01){
            if(iLoop == iflash_count + 1){ mPaintSpeed.setColor(COLOR_SPEED_ALPHA); }
        }
        if((ieffect & 0x02) == 0x02){
            if(iLoop == iflash_count){
                if(icount%8 >= 4){ bret=false; }
            }
        }
        return bret;
    }
        ...
        if(Speed_list != null){
            ...
            for (iLoop=0; iLoop < iXScale+1; iLoop++){
                if(iLoop < Speed_list.size()){
                    ...
                    iPosY=iViewHeight-(int)(fspeed*iSingleHeight)-iBox_h/2;
                    iEndPosY=iPosY+iBox_h;
                    rect=new RectF(iPosX, iPosY, iEndPosX, iEndPosY);
                    if(vSpeed_effect(iLoop)){ canvas.drawRoundRect(rect, iGet_Scale(15 / 2), iGet_Scale(15 / 2), mPaintSpeed); }
                }
            }
        }
    }
    ...
    private boolean vIncline_effect(int iLoop){
        ...
        if((ieffect & 0x01) == 0x01){
            if (iLoop == iflash_count+1){ mPaintIncline.setColor(COLOR_INCLINE_ALPHA); }
        }
